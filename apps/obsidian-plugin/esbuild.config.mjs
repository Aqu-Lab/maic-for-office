 ...
 *
 * Output: dist/main.js (+ manifest.json + styles.css copied alongside). The canonical
 * manifest.json / versions.json live at the REPOSITORY ROOT (the Obsidian directory
 * listing validates them there); this script bundles them into dist/ for release.
 * When OBSIDIAN_TEST_VAULT (env) or testVault (package.json "officeViewer" key) points
 * at a vault, the three files are also copied into <vault>/.obsidian/plugins/maic-for-office/
 * so a Community-plugins reload picks the build up immediately.
 *
 * Engine packages (packages/docx-engine, pptx-engine, pptx-render) are consumed as
 * workspace source (their exports point at src/index.ts), so esbuild compiles everything
 * into the single main.js Obsidian expects.
 */
