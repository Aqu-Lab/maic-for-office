...
在 Obsidian 中直接查看 vault 里的 `.docx`、`.pptx`、`.xlsx` 附件——解析与渲染跑在 GenOffice 引擎包上，完全本地，不联网、不上传。
...
本项目 fork 自 [GenOffice](https://github.com/genspark-ai/genoffice)（Apache-2.0）。三个格式共用其引擎：

- **docx**：解析层与 Docs 应用完全同源；渲染层是为只读场景新写的 DOM 输出（Docs 应用是可编辑 Tiptap 分页视图）。
- **pptx**：渲染树构建与 Slides 应用同源；Konva 静态绘制层（`render/pptx/`）是从 Slides renderer 复制的闭包，后续 Slides 有渲染修复时可同步。
- **xlsx**：原应用的解析走 Rust sidecar（无法随插件分发），此处改用 SheetJS CE，样式保真度低于 Sheets 应用。
...
- **pptx 字体 metrics 为启发式**：当前用 `HeuristicMetrics` 估行宽；接入系统字体 + opentype.js（参照 `apps/slides/src/main/fonts.ts`）是下一步保真度重点。
- **xlsx 只显示值**：样式、图表、透视表不渲染；超大表按设置页上限截断。

## 许可

Apache-2.0（继承上游 GenOffice）。构建产物 `main.js` 内嵌了同仓库引擎包的编译代码，对应源码均在本仓库内。
